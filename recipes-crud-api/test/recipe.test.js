...
describe("test the recipes API", () => {
  ...
  describe("POST/recipes", () => {
    ...
    it("it should not save new recipe to db,incalid vegetarian value", async () => {
      // DATA YOU WANT TO SAVE TO DB
      const recipe = {
        name: "chicken nuggets",
        difficulty: 3,
    ...
    it("it should not save new recipe to db,invalid token", async () => {
      ...
      const res = await request(app).post("/recipes").send(recipe).set("Authorization", "Bearer siuogusire7893034pt4");
  ...
  describe("GET/recipes/:id", () => {
    ...
    it("it should not retrieve any recipe from the db,invalid id passed", async () => {
      const res = await request(app).get("/recipes/282hsowj8byewiuewgfg6732i2");
      expect(res.statusCode).toEqual(400);
      expect(res.body).toEqual(
        expect.objectContaining({
          success: false,
